...
class UserBillManager(models.Manager):
    ...
    def generate_bill(self, user, billing_from, billing_to):
        groups = Group.objects.filter(members=user)
        total_amount = 0
        # calculate user cost per group and add it to total_amount variable
        for group in groups:
            bills = Bill.objects.filter(group=group, spend_on__range=(billing_from, billing_to)).aggregate(total=models.Sum('amount'))
        ...
        monthly_bill, created = MonthlyUserBill.objects.get_or_create(user=user, billing_from=billing_from, billing_to=billing_to)
        spend_bills = Bill.objects.filter(spend_by=user, spend_on__range=(billing_from, billing_to)).aggregate(total=models.Sum('amount'))
...
class MonthlyUserBill(models.Model):
    ...
    def __unicode__(self):
        return '{0} - {1} - {2}'.format(self.user.username, self.billing_from, self.billing_to)
